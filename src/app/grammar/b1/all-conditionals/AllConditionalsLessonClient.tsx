...
export default function AllConditionalsLessonClient() {
  ...
  const sets: Record<1 | 2 | 3 | 4 | 5, ExerciseSet> = useMemo(() => ({
    ...
    5: {
      ...
      instructions: "Read the story. Open the brackets using the correct conditional form: zero, first, or second.",
      passage: "It is the middle of February. Tom and his sister Kate are planning their summer holiday, but they cannot agree on a destination.\n\nTom wants to go to Norway. He says: \"If you (1)(visit) Norway in summer, the sun never really sets — it is an incredible experience.\" That is just a fact about Norway in June.\n\nKate prefers Greece. She argues: \"If we (2)(book) the flights this week, we will get a much better price.\" Tom agrees — early booking always saves money.\n\nTom worries about the cost. \"If we (3)(go) to Norway, it would be very expensive. We would need a much bigger budget.\" He is imagining a hypothetical.\n\nKate has a practical plan: \"If I (4)(get) a bonus at work next month, I will contribute more to the holiday fund.\" She is fairly confident this will happen.\n\nTom nods and adds: \"If we (5)(have) more money, we could visit both countries.\" He smiles — that would be the ideal, though unlikely, scenario.\n\nIn the end they agree: if the weather forecast for Greece (6)(look) good in June, they (7)(book) the trip. If it (8)(be) a success, they will make it an annual tradition.",
      questions: [
        { id: "e5q1", prompt: "(1) If you _____ (visit) Norway in summer", correct: "visit", explanation: "Zero conditional — general fact about Norway: if + present, present simple." },
        { id: "e5q2", prompt: "(2) If we _____ (book) flights this week", correct: "book", explanation: "First conditional if-clause → present simple: book." },
        { id: "e5q3", prompt: "(3) If we _____ (go) to Norway", correct: "went", explanation: "Second conditional if-clause → past simple: went (hypothetical cost scenario)." },
        { id: "e5q4", prompt: "(4) If I _____ (get) a bonus next month", correct: "get", explanation: "First conditional if-clause → present simple: get (real possibility)." },
        { id: "e5q5", prompt: "(5) If we _____ (have) more money", correct: "had", explanation: "Second conditional if-clause → past simple: had (hypothetical/unlikely scenario)." },
        { id: "e5q6", prompt: "(6) if the weather forecast _____ (look) good", correct: "looks", explanation: "First conditional if-clause → present simple: looks." },
        { id: "e5q7", prompt: "(7) they _____ (book) the trip", correct: "will book", explanation: "First conditional result clause → will + infinitive: will book." },
        { id: "e5q8", prompt: "(8) If it _____ (be) a success", correct: "is", explanation: "Zero/first conditional — general truth about annual traditions: present simple." },
      ],
    },
  }), []);
  ...
  function switchExercise(n: 1 | 2 | 3 | 4 | 5) { window.scrollTo({ top: 0, behavior: "smooth" }); setExNo(n); setChecked(false); setMcqAnswers({}); setInputAnswers({}); broadcast({ answers: {}, inputAnswers: {}, checked: false, exNo: n }); }
                            ...
                            <div key={q.id} className="rounded-2xl border border-black/10 bg-white p-5">
                              <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1">
                                  <div className="font-bold text-slate-900">{q.prompt}</div>
                                  <div className="mt-3">
                                    <input value={val} disabled={checked} onChange={(e) => { const n = { ...inputAnswers, [q.id]: e.target.value }; setInputAnswers(n); broadcast({ answers: mcqAnswers, inputAnswers: n, checked, exNo }); }} placeholder="Type the correct form…" className="w-full max-w-sm rounded-xl border border-black/10 bg-white px-4 py-3 text-slate-900 placeholder:text-slate-400 outline-none focus:border-[#F5DA20]" />
                                  </div>
                                  {checked && (
                                    <div className="mt-3 text-sm">
                                      {isCorrect && <div className="text-emerald-700 font-semibold">✅ Correct</div>}
