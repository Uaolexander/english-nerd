...
type Props = {
  isLoggedIn: boolean;
  level: string;          // "A1", "B1", "B2", "C1"
  wordBank: string[];
  exercises: Exercise[];
  loginRedirect: string;  // where to send user after login
...
export default function DownloadWorksheet({ isLoggedIn, level, wordBank, exercises, loginRedirect, filename }: Props) {
      ...
      <div ref={page1Ref} style={{ position: "fixed", left: -9999, top: 0, width: PAGE_W, height: PAGE_H, backgroundColor: "#fff", overflow: "hidden", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ flex: 1, padding: "28px 32px 0", display: "flex", flexDirection: "column" }}>
          <div style={{ marginBottom: 20 }}>
            <h1 style={{ ...C, fontSize: 30, fontWeight: 900, color: "#0f172a", margin: 0 }}>Phrasal Verbs</h1>
          ...
          <div style={{ border: "2px dashed #94a3b8", borderRadius: 12, marginBottom: 16, overflow: "hidden" }}>
            <div style={{ backgroundColor: "#f1f5f9", borderBottom: "1px solid #e2e8f0", padding: "8px 16px" }}>
              <span style={{ ...C, fontSize: 10, fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.1em", color: "#475569" }}>Word Bank — use each verb once</span>
            </div>
          ...
          <p style={{ ...C, fontSize: 12, fontStyle: "italic", color: "#64748b", marginBottom: 16 }}>Fill in each blank with the correct phrasal verb from the word bank above. Use each verb once. Change the form if necessary.</p>
            ...
            {exercises.map(({ before, after }, i) => (
              <li key={i} style={{ display: "flex", gap: 10, marginBottom: 14, alignItems: "flex-start" }}>
                ...
                <span style={{ ...C, fontSize: 14, color: "#1e293b", lineHeight: 1.6 }}>
                  {before}<span style={{ display: "inline-block", width: 120, borderBottom: "2px solid #475569", verticalAlign: "bottom", marginLeft: 2, marginRight: 2 }} />{after}
                </span>
              </li>
            ))}
          </ol>
        ...
        <div style={{ flex: 1, padding: "28px 32px", overflowY: "hidden" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
            ...
            <div>
              ...
              <p style={{ ...C, fontSize: 12, color: "#64748b", margin: "2px 0 0" }}>Phrasal Verbs — Fill in the Blanks · {level}</p>
            </div>
          </div>
